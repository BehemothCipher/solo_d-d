...
function buildImagePrompt(narration) {
  ...
  let base = "dark fantasy RPG digital art, cinematic painterly style, dramatic lighting, detailed, no text, no watermark";
  let scene = "";
  ...
  else if (/cave|tunnel|underground|cavern|dark|passage/.test(l))
...
function getImageUrl(narration) {
  ...
  const prompt = buildImagePrompt(narration);
  return `https://image.pollinations.ai/prompt/${prompt}?width=1280&height=720&nologo=true&model=flux&enhance=true&seed=${seed}`;
}
...
.die-btn{background:${S.rune};border:1px solid ${S.border};color:${S.gold};font-family:'Cinzel',serif;font-size:9px;padding:4px 7px;border-radius:3px;cursor:pointer;transition:all .15s;}
...
.scene-label{position:absolute;bottom:8px;left:10px;font-size:9px;color:rgba(180,200,220,.6);font-style:italic;z-index:1;font-family:'Cinzel',serif;letter-spacing:.05em;}
...
function SceneImage({ narration }) {
  ...
  return (
    <div className="scene-wrap">
      ...
      {errored && (
        <div style={{ padding: "20px", textAlign: "center", fontSize: 11, color: S.muted, fontStyle: "italic" }}>
          Scene image unavailable
        </div>
      )}
      {loaded && <div className="scene-label">Craghaven Mountains</div>}
    </div>
  );
}
      ...
      <div className="content">
        ...
        <div className="main">
          ...
          <div className="log" ref={logRef}>
            {messages.map((msg, i) => {
              if (msg.type === "scene")  return <SceneImage key={i} narration={msg.narration} />;
